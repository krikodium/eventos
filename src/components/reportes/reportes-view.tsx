...
type ReporteData = {
  ...
  eventosSinTipoCambio: Array<{ id: string; nombre: string }>;
  totales: {
    pagosProveedores: number;
    utileros: number;
    cajaChica: number;
    ...
    <div className="space-y-6">
      ...
      {data ? (
        <div className="space-y-6">
          ...
          {data.eventosSinTipoCambio.length > 0 && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900">
              <p className="font-semibold">
                {data.eventosSinTipoCambio.length} evento
                {data.eventosSinTipoCambio.length === 1 ? " quedó" : "s quedaron"} fuera del reporte
              </p>
              <p className="mt-1 text-amber-800">
                Tienen movimientos USD sin tipo de cambio: {data.eventosSinTipoCambio.map((evento) => evento.nombre).join(", ")}.
              </p>
            </div>
          )}
